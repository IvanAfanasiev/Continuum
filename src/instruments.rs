...
impl InstrumentState {
    ...
    fn synth_piano(&mut self, freq: f32, sr: f32) -> f32 {
        ...
        let hammer_click = if self.t < 0.012 {
            self.white_noise() * (1.0 - self.t / 0.012).powi(2) * 0.006
        } else {
            0.0
        };

        let key_weight = if self.t < 0.020 {
            (self.t * 42.0 * 2.0 * PI).sin() * (1.0 - self.t / 0.020).powi(2) * 0.018
        } else {
            0.0
        };

        let raw = (out + hammer_click + key_weight) * 0.42;
        self.low_pass(raw, 2600.0, sr)
    }
    ...
    fn synth_bass(&mut self, freq: f32, sr: f32) -> f32 {
        let pitch_envelope = 1.0 + 0.035 * (-45.0 * self.t).exp();
